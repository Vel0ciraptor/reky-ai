...
export class ChatService {
    ...
    async sendMessage(data: {
        ...
        const senderCount = previousMsgs.filter(m => m.senderId === data.senderId).length;
        const receiverCount = previousMsgs.filter(m => m.senderId === data.receiverId).length;
        ...
        if (!canShareContact) {
            ...
            if (emailRegex.test(data.content) || phoneRegex.test(data.content)) {
                return { error: 'Por seguridad, no puedes enviar correos electrónicos o números telefónicos hasta que al menos uno haya enviado 5 mensajes.' };
            }
        }
        ...
        try {
            ...
            return { error: 'Saldo insuficiente. Recarga tu wallet para seguir enviando mensajes.' };
        }
        ...
        const message = await this.prisma.message.create({
            ...
            include: {
                sender: { select: { id: true, name: true, lastName: true, avatarUrl: true } },
                receiver: { select: { id: true, name: true, lastName: true, avatarUrl: true } },
            },
        });
    ...
    async getConversations(agentId: string) {
        ...
        const allMessages = await this.prisma.message.findMany({
            ...
            include: {
                sender: { select: { id: true, name: true, lastName: true, avatarUrl: true, role: true } },
                receiver: { select: { id: true, name: true, lastName: true, avatarUrl: true, role: true } },
            },
