...
export class SearchService {
    ...
    async search(filters: {
        ...
        if (filters.dormitorios) where.dormitorios = { gte: Number(filters.dormitorios) };
        if (filters.banos) where.banos = { gte: Number(filters.banos) };
        if (String(filters.estacionamiento) === 'true' || filters.estacionamiento === true) where.estacionamiento = true;
        if (String(filters.piscina) === 'true' || filters.piscina === true) where.piscina = true;
        if (String(filters.patio) === 'true' || filters.patio === true) where.patio = true;
        ...
        if (String(filters.mascotas) === 'true' || filters.mascotas === true) where.mascotas = true;

        if (filters.tipoVivienda) {
            where.tipoVivienda = { contains: filters.tipoVivienda, mode: 'insensitive' };
        }
        ...
        if (filters.construccionMin || filters.construccionMax) {
            where.construccion = {};
            if (filters.construccionMin) where.construccion.gte = Number(filters.construccionMin);
            if (filters.construccionMax) where.construccion.lte = Number(filters.construccionMax);
        }
