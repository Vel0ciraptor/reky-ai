...
export class AdminService {
    ...
    async getDashboardMetrics() {
        ...
        const [
            ...
            this.prisma.transaction.findMany({
                where: { verificado: true, fecha: { gte: new Date(now.getFullYear() - 1, now.getMonth(), 1) } },
        ...
        const monthlyChart: { label: string; ventas: number; propiedades: number }[] = [];
        for (let i = 11; i >= 0; i--) {
        ...
        const allRecentTxs = await this.prisma.transaction.findMany({
            where: { verificado: true, fecha: { gte: new Date(now.getFullYear(), now.getMonth(), now.getDate() - 28) } },
            select: { fecha: true },
        });
        ...
        for (let i = 27; i >= 0; i--) {
            ...
            const dayEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate() - i + 1, 0, 0, 0);
            const label = dayStart.toLocaleDateString('es-BO', { day: '2-digit', month: 'short' });
            const ventas = allRecentTxs.filter(t => t.fecha >= dayStart && t.fecha < dayEnd).length;
        ...
        const weeklyChart: { label: string; ventas: number; propiedades: number }[] = [];
        for (let i = 11; i >= 0; i--) {
            ...
            const ventas = allWeeklyTxs.filter(t => t.fecha >= weekStart && t.fecha < weekEnd).length;
    ...
    async getPendingTransactions() {
        return this.prisma.transaction.findMany({
            ...
            include: {
                agent: { select: { id: true, name: true, lastName: true, email: true, avatarUrl: true } },
                property: {
                    select: {
                        id: true, matricula: true, ubicacion: true, tipo: true, precio: true,
    ...
    async getPropertyHistory() {
        return this.prisma.property.findMany({
            include: {
                agents: {
                    include: { agent: { select: { id: true, name: true, lastName: true } } },
                },
                transactions: {
    ...
    async getPendingProperties() {
        ...
        return this.prisma.property.findMany({
            ...
            include: {
                agents: {
                    include: {
                        agent: { select: { id: true, name: true, lastName: true, email: true, avatarUrl: true, agencyId: true, agency: { select: { name: true } } } },
                    },
                },
                images: { take: 1, select: { url: true } },
            },
    ...
    async generateCsvReport(): Promise<string> {
        ...
        addRow(['FECHA', 'TIPO OPERACION', 'ESTADO', 'PRECIO (USD)', 'MATRICULA', 'UBICACION/PROPIEDAD', 'AGENTE', 'AGENCIA']);

        for (const tx of transactions) {
            const date = new Date(tx.fecha).toLocaleDateString('es-BO', { day: '2-digit', month: '2-digit', year: 'numeric' });
            ...
            addRow([date, type, status, price, matricula, location, agentName, agencyName]);
        }
