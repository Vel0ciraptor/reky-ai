...
export class AgenciesService {
    ...
    async getDashboard(agentId: string) {
        const requester = await this.prisma.agent.findUnique({
            where: { id: agentId },
            select: { id: true, role: true, agencyId: true, name: true, lastName: true },
        });

        if (!requester || requester.role !== 'agencia') {
            throw new ForbiddenException('Solo las agencias pueden acceder a este panel.');
        }
        ...
        if (!agencyId) {
            ...
            await this.prisma.agent.update({ where: { id: agentId }, data: { agencyId: agency.id } });
            agencyId = agency.id;
        }
        ...
        const agents = await this.prisma.agent.findMany({
            ...
            select: {
                ...
                role: true, verified: true, emailVerified: true, points: true,
            ...
            include: {
                agent: {
                    select: {
                        ...
                        phone: true, avatarUrl: true, points: true,
                        _count: { select: { properties: true, transactions: true } },
                    },
                },
            },
        ...
        // Aggregate stats
        const totalProperties = agents.reduce((sum, a) => sum + (a._count?.properties ?? 0), 0);
        const totalTransactions = agents.reduce((sum, a) => sum + (a._count?.transactions ?? 0), 0);
        const totalPoints = agents.reduce((sum, a) => sum + a.points, 0);
        const totalBalance = agents.reduce((sum, a) => sum + Number(a.wallet?.balance ?? 0), 0);
    ...
    async requestToJoin(agentId: string, agencyId: string) {
        ...
        if (agent.agencyId) throw new ConflictException('Ya perteneces a una agencia.');
        ...
        if (existing) {
            if (existing.status === 'pending') throw new ConflictException('Ya tienes una solicitud pendiente.');
    ...
    async acceptRequest(requesterId: string, requestId: string) {
        ...
        if (!requester || requester.role !== 'agencia') {
            throw new ForbiddenException('Solo las agencias pueden aceptar solicitudes.');
        }
    ...
    async rejectRequest(requesterId: string, requestId: string) {
        ...
        if (!requester || requester.role !== 'agencia') {
            throw new ForbiddenException('Solo las agencias pueden rechazar solicitudes.');
        }
    ...
    async inviteAgent(requesterId: string, email: string) {
        ...
        if (!target) throw new NotFoundException('No existe un agente con ese correo.');
        if (target.agencyId) throw new ConflictException('Este agente ya pertenece a una agencia.');
        if (target.role === 'agencia') throw new ConflictException('No puedes invitar a otra agencia.');

        // Check for existing request/invitation
        const existing = await this.prisma.agencyRequest.findUnique({
            where: { agentId_agencyId: { agentId: target.id, agencyId: requester.agencyId } },
        });
        if (existing) {
            if (existing.status === 'pending') throw new ConflictException('Ya existe una solicitud/invitación pendiente.');
            // Update if previously rejected
            await this.prisma.agencyRequest.update({
                where: { id: existing.id },
                data: { status: 'pending', type: 'invitation', createdAt: new Date() },
            });
            return { message: `Invitación reenviada a ${target.name} ${target.lastName}.` };
        }
        ...
        return { message: `Invitación enviada a ${target.name} ${target.lastName}.` };
    }
    ...
    async acceptInvitation(agentId: string, requestId: string) {
        const request = await this.prisma.agencyRequest.findUnique({ where: { id: requestId } });
        if (!request || request.agentId !== agentId || request.type !== 'invitation') {
    ...
    async removeAgent(requesterId: string, agentId: string) {
        ...
        if (!requester || requester.role !== 'agencia') {
            throw new ForbiddenException('Solo las agencias pueden gestionar agentes.');
        }
    ...
    async getAgencyProperties(requesterId: string) {
        ...
        const relations = await this.prisma.propertyAgent.findMany({
            ...
            include: {
                ...
                agent: { select: { id: true, name: true, lastName: true, avatarUrl: true } },
            },
            orderBy: { createdAt: 'desc' },
        });
